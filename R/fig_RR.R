...
alt <- as.data.frame(
	t(sapply(results, function(x) c(x$param,x$actual[2,1:6])))
)

p_alt <- as.data.frame(
	t(sapply(results, function(x) c(x$param,
		mean=p_func(x$actual[2,"mean"],x$null2[,"mean"]),
		median=p_func(x$actual[2,"median"],x$null2[,"median"]),
		mode= p_func(x$actual[2,"mode"],x$null2[,"mode"])
		# ,x$actual["LRT"]
		)))
)

load( file=paste0(wd,"Data/Intermediate/RR_sim_null.Rdata"))
null <- as.data.frame(
	t(sapply(results, function(x) c(x$param,x$actual[2,1:6])))
)
null$sim_var1 <- 0
...
long_alt <- Stack(alt,c("mean","median","mode"), group.name="type",value.name = "estimate")
long_null <- Stack(null,c("mean","median","mode"), group.name="type",value.name = "estimate")

p_long_alt <- Stack(p_alt,c("mean","median","mode"), group.name="type",value.name = "estimate")#,"LRT"
p_long_null <- Stack(p_null,c("mean","median","mode"), group.name="type",value.name = "estimate")#,"LRT"
...
beeswarm(estimate~sim_var2+type,long, pch=19, cex=0.5, col=alpha(1,0.3),method = "compactswarm",corral="wrap",xlab="Between group variance", ylab="Estimate", labels=rep(c(0,0.2),4))#, 
mtext(c("ML","Post. Mean","Post. Median","Post. Mode"),side=3,adj=c(0.1,0.35,0.65 ,0.9))
means<-aggregate(estimate~sim_var2+type,long,mean)$estimate
CIs<-aggregate(estimate~sim_var2+type,long,function(x) sd(x)/sqrt(length(x)))$estimate*qnorm(0.975)
...
arrows(c(2,4,6,8)-0.4,0.1,c(2,4,6,8)+0.4,0.1,code=0, angle=90, length=0.05, col="red", lwd=2)
abline(v=c(2.5,4.5,6.5))
# mtext(c("ML","Posterior Mean","Posterior Mode"),side=3,adj=c(0.15,0.5, 0.85))
mtext("a)",2,padj=-8, las=1, line=2)


beeswarm(estimate~sim_var2+type,p_long, pch=19, cex=0.5, col=alpha(1,0.3),method = "compactswarm",corral="wrap",xlab="Between group variance", ylab=bquote(P[perm]), labels=rep(c(0,0.2),4))#, labels=c("ML","Posterior Mean","Posterior Median","Posterior Mode"))
# boxplot(estimate~sim_var2+type,p_long, pch=19, cex=0.5,xlab="Between group variance", ylab=bquote(P[perm]),boxwex=0.5, names=rep(c(0,0.2),4))#, 
abline(v=c(2.5,4.5,6.5))
mtext(c("ML","Post. Mean","Post. Median","Post. Mode"),side=3,adj=c(0.1,0.35,0.65 ,0.9))
