...
beeswarm(est~type, res, pch=19, cex=0.1, col=alpha(1,0.3),method = "compactswarm",corral="wrap",xlab="", ylab="Variance estimate",  ylim=c(0,0.15), xaxt="n")
mtext("a)",2,padj=-8, las=1, line=2)
abline(h=obs_est_sim[4], col="red")

text(1:6,0.14,tapply(res$est,res$type,function(x)mean(x>obs_est_sim["sigma2_I2"])), cex=0.75)


par(mar=c(3.5,4,1,1))
beeswarm(est~type, res_GT, pch=19, cex=0.1, col=alpha(1,0.3),method = "compactswarm",corral="wrap", ylab="Variance estimate",  ylim=c(0,0.15), xlab="", xaxt="n")#
...
text(1:6,0.14,tapply(res_GT$est,res_GT$type,function(x)mean(x>obs_est_real["sigma2_I2"])), cex=0.75)
