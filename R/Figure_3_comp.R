...
# biasM<-aggregate(cbind(mode0.1_bias,mode1_bias,mean_bias,median_bias)~ICC+N_group+N_within,actual,median)
...
# abs_biasM<-aggregate(cbind(mode0.1_bias,mode1_bias,mean_bias,median_bias)~ICC+N_group+N_within,actual,function(x) median(abs(x)))
...
par(mar=c(4,4,1,1))
plot(relative_bias$mean_rel_bias, pch=19, ylim=c(-3,3), ylab="Absolute Bias", xaxt="n", xlab="ICC")
abline(h=0)
abline(v=line_coords, lty=c(2,2,1,2,2))
points(relative_bias$median_rel_bias, pch=19,col="red")
points(relative_bias$mode1_rel_bias, pch=19,col="blue")
points(relative_bias$mode0.1_rel_bias, pch=19,col="green")
